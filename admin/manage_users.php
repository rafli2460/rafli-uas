                        ...
                        <td>
                            <?php if ($user['id'] != $_SESSION['id']): // Prevent admin from editing self in this simple form ?>
                                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" style="display: inline-block;">
                                    ...
                                    <button type="submit" name="change_role" class="btn btn-primary">Save Role</button>
                                </form>
                                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" style="display: inline-block;">
                                    <input type="hidden" name="user_id" value="<?php echo $user['id']; ?>">
                                    <button type="submit" name="delete_user" class="btn btn-danger" onclick="return confirm('Are you sure? This will delete the user and all their posts.');">Delete</button>
                                </form>
                            <?php else: ?>
